...
A complete ecommerce project for the local Indian store "BuyinHome - Khadrido", with a React frontend and Cloudflare Workers backend in one repository.
...
This frontend is designed to connect to the backend API at `VITE_API_URL`.
...
- `POST /api/order/pay` - Process payment
- `GET /api/track/:orderRef` - Order status
...
backend/
├── src/                 # Cloudflare Worker routes and utilities
